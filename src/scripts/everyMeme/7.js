...
loadImg().then(() => {
    ...
    if (ctx.measureText(text1).width > canvas.width / 2) {
        text1 = splitTextToFitCanvas(text1, 175);
    }

    // Calcula la posición X para que el texto empiece desde la mitad del canvas
    const x1 = 50; // La mitad del canvas dividida por 2

    // Dibuja el texto en la posición X calculada
    drawWrappedText(ctx, text1, x1, 85, canvas.width / 2 + 15, 30);
    ...
    if (ctx.measureText(text2).width > canvas.width / 2) {
        text2 = splitTextToFitCanvas(text2, canvas.width / 2 - 5);
    }

    // Calcula la posición X para que el texto empiece desde la mitad del canvas
    const x2 = canvas.width / 2 + 15; // La mitad del canvas dividida por 2

    // Dibuja el texto en la posición X calculada
    drawWrappedText(ctx, text2, x2, canvas.height - 30, canvas.width / 2 + 15, 30);
});

async function loadImg() {
    return new Promise((resolve, reject) => {
        let img = new Image();
        img.src = "./src/img/template-7.jpg";
