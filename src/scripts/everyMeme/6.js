...
loadImg().then(() => {
    ...
    if (ctx.measureText(text1).width > canvas.width / 2) {
        text1 = splitTextToFitCanvas(text1, canvas.width / 2 - 5);
    }

    // Calcula la posición X para que el texto empiece desde la mitad del canvas
    const x1 = 0; // La mitad del canvas dividida por 2
    ...
    // Calcula la posición X para que el texto empiece desde la mitad del canvas
    const x2 = 0; // La mitad del canvas dividida por 2
...
async function loadImg() {
    return new Promise((resolve, reject) => {
        let img = new Image();
        img.src = "./src/img/template-6.jpg";
